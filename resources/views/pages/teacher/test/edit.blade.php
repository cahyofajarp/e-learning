...
<form action="{{ route('teacher.lesson.test.update',[$classroom,$lesson,$test]) }}" method="post" id="formData">
    @csrf
    @method('put')
    <div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-12">  
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="test_code" class="form-control" value="{{ old('test_code',$test->test_code) }}">
                                <span class="text-danger error-text test_code_error" style="font-size:13px"></span>
                            </div> 
                            <div class="form-group">
                                <label for="">* Nama Ujian</label>
                                <input type="text" name="name" class="form-control" value="{{ old('test_code',$test->name) }}">
                                <span class="text-danger error-text name_error" style="font-size:13px"></span>
                            </div> 
                                <div class="form-group">
                                    <label for="">* Nilai KKM</label>
                                    <input type="number" name="standard" class="form-control" value="{{ old('standard',$test->standard) }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="time" class="form-control" value="{{ old('time',$test->time) }}">
                                    <span class="text-danger error-text time_error" style="font-size:13px"></span>
                                </div>
                                <div class="form-group">
                                    <label for="">* Deskripsi (Opsional)</label><br>
                                    <input type="text" name="decription" class="form-control" value="{{ old('decription',$test->time) }}">
                                ...
                                @foreach ($levelclass->classrooms as $class)
                                    @if ($class->name != $classroom->name)
                                        <option value="{{ $class->id }}" 
                                            @foreach ($test->classrooms()->get() as $classtest)
                                                @if ($class->id == $classtest->id)
                                                    selected
                                                @endif
                                            @endforeach
                                            >{{ $class->name }} </option>
