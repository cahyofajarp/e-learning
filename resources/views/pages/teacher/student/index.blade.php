                            ...
                            @foreach ($lessonTeacher as $lessonteacher)
                                @foreach ($lessonteacher->classroom()->get() as $classroom)
                                    <tr>
                                        ...
                                        <td><span class="btn btn-warning"> {{ $classroom->students()->count() }} Murid </span></td>
                                        <td>
                                            <a href="{{ route('teacher.student.studentAll',$classroom) }}" class="btn btn-custom-green btn-sm py-2 px-3" style="border-radius: 100px"><i class="mdi mdi-eye"></i> Lihat detail Siswa</a>
                                        </td>
                                    </tr>
                                <?php $no++ ?>
                                @endforeach
