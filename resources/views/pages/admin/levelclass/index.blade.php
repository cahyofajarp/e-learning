                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="">Name</label>
                            <small class="d-block mb-4 text-gray">Noted : Silahkan isi wajib angka ,contoh : 10 (Maksud nya adalah kelas 10) </small>
                            <input type="number" name="name" class="form-control" placeholder="Level class : 10,11,12">
                            <span class="text-danger error-text name_error" style="font-size: 13px"></span>
                        </div>
                    </div>
                </div>    
        ...
        <div class="card">
            <div class="card-body">
                ...
                <table class="table table-hover table-striped" id="myTable">
                    ...
                    <tbody>
                        <?php $no = 1;  ?>
                        @foreach ($levelclasses->sortByDesc('name') as $levelclass)
                            <tr>    
                                <td class="text-center">{{ $no }}</td>
                                <td>{{ $levelclass->name }}</td>
                                <td>
                                    <button data-update="{{ route('admin.levelclass.update',$levelclass) }}" data-url="{{ route('admin.levelclass.edit',$levelclass) }}" id="btn-edit" href="" class="btn btn-sm btn btn-custom-yellow">
                                        <i class="mdi mdi-pencil"></i> Edit
                                    </button>
                                    <button data-delete="{{ route('admin.levelclass.destroy',$levelclass) }}" id="btn-delete" class="btn btn-custom-red btn-sm btn-danger" style="color:white">
...
$('#formDataCreate').submit(function(e) {
    ...
    $.ajax({
        ...
        contentType: false,
        processData: false,
        url: "{{ route('admin.levelclass.store') }}",
        beforeSend:function(){
